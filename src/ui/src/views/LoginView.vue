...
async function handleLogin() {
  ...
  try {
    const response = await api.post<{ token: string; user: any; csrf_token?: string; refresh_token?: string }>('/api/v1/auth/login', {
      email: email.value,
...
async function handleRegister() {
  ...
  try {
    const response = await api.post<{ token: string; user: any; csrf_token?: string; refresh_token?: string }>('/api/v1/auth/register', {
      email: email.value,
      password: password.value,
      name: name.value,
    })
